...
class GDF:
    def __init__(self, all_gdf_data, gm_api_key, request_headers):
        ...
        if not self.intersection_layer.empty:
            for idx, intersection in self.intersection_layer.iterrows():
                rd_id_lst = self.get_rd_id_lst(intersection, 9)
                self.intersection_nodes.append(
                    IntersectionNode(intersection["CRE_DATE"], intersection["INT_ID"], intersection["INT_TYPE"], intersection["INT_ENAME"], intersection["INT_CNAME"], rd_id_lst, intersection["REMARKS"], intersection["LAST_UPD_DATE_V"], intersection["SHAPE"])
                )

        """ ROUNDABOUT NODES """
        self.roundabout_nodes = []
        if not self.roundabout_layer.empty:
            for idx, rdb in self.roundabout_layer.iterrows():
                rd_id_lst = self.get_rd_id_lst(rdb, 9)
                self.roundabout_nodes.append(
                    RoundaboutNode(
                        rdb["SHAPE"], rdb["CRE_DATE"], rdb["R_ABOUT_ID"], rdb["R_ABOUT_ENAME"], rdb["R_ABOUT_CNAME"], rdb["R_ABOUT_TYPE"], rdb["GRADED"], rdb["SIGNALIZED"], rdb["NO_OF_ARM"], rd_id_lst, rdb["REMARKS"], rdb["LAST_UPD_DATE_V"]
                    )
                )

        """ TRAFFIC NODES """
        self.traffic_nodes = []
        if not self.traffic_features_layer.empty:
            for idx, traffic_feature in self.traffic_features_layer.iterrows():
                rd_id_lst = self.get_rd_id_lst(traffic_feature, 8)
                self.traffic_nodes.append(
                    TrafficNode(
                        traffic_feature["SHAPE"], traffic_feature["FEATURE_TYPE"], traffic_feature["FEATURE_ID"], traffic_feature["CRE_DATE"], rd_id_lst, traffic_feature["REMARKS"], traffic_feature["TUN_BRIDGE_ID"], traffic_feature["LAST_UPD_DATE_V"]
                    )
                )

        """ BUS ONLY NODES """
        self.bus_only_nodes = []
        if not self.bus_only_lane_layer.empty:
            for idx, bo_lane in self.bus_only_lane_layer.iterrows():
                self.bus_only_nodes.append(
                    BusOnlyNode(
                        bo_lane["ROAD_ROUTE_ID"], bo_lane["TIME_ZONE"], bo_lane["EFFECTIVE_DAY"], bo_lane["BOUND"], bo_lane["REMARKS"], bo_lane["BUS_ONLY_LANE_ID"], bo_lane["CRE_DATE"], bo_lane["LAST_UPD_DATE_V"], bo_lane["SHAPE"], bo_lane["SHAPE_Length"]
                    )
                )

        """ TUNBRI TOLL NODES """
        self.tunbri_toll_nodes = []
        if not self.tun_bridge_toll_layer.empty:
            for idx, tunbri in self.tun_bridge_toll_layer.iterrows():
                self.tunbri_toll_nodes.append(
                    TunnelBridgeNode(
                        tunbri["TUNNEL_BRIDGE_NAME"], tunbri["TUNNEL_BRIDGE_CHINESE_NAME"], tunbri["FEATURE_ID_1"], tunbri["FEATURE_ID_2"], tunbri["EFFECTIVE_DATE"], tunbri["GAZETTED_TOLL"], tunbri["CONCESSION_TOLL"], tunbri["VEHICLE_CLASS_DESCRIPTION"], tunbri["REMARKS"], tunbri["LAST_UPDATED_DATE"]
                    )
                )
    ...
    def get_rd_id_lst(self, parent_dict: dict, range_idx: int):
        rd_id_lst = []
        for idx in range(0, range_idx+1):
            rd_id_lst.append(
                parent_dict[f"RD_ID_{idx}"]
            )
        return rd_id_lst
